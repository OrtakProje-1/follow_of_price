...
class TotalPrice extends StatefulWidget {
  ...
  const TotalPrice({Key? key, required this.month,required this.type,this.textSize=25}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Price>>(
      ...
      builder: (context, snapshot) {
        ...
        return Row(
          children: [
            Expanded(child: Const.buildContent( "₺"+amount.toStringAsFixed(2),textSize: 20,minSize: 10)),
          ],
        );
      },
    );
